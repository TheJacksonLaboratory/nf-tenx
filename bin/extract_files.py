...
annot_paths = (path for path in args.gene_annotations_dir.iterdir() if path.stem in str(args.ref_genome_dir))
...
dir_to_pattern = {Path.cwd(): '*filtered*matrix*.h5', args.ref_genome_dir: '*genes*.gtf'}
for direc, pattern in dir_to_pattern.items():
    paths = list(direc.glob(f'**/{pattern}'))
    if len(paths) != 1:
        raise ValueError(f'{", ".join(str(path) for path in paths)} were found to match the glob pattern {pattern}. This pipeline requires exactly one file of this type to match this pattern.')
    actual_path = paths[0]
